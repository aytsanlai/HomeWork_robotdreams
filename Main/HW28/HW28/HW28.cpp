...
int main()
{
    ...
    std::vector<std::thread> npcThreads;
    ...
    while (window.isOpen())
    {
        ...
        for (GameObject* npc : gameWorld->getNPCs()) {
            AIMoveBestRouteComponent* aiComponent = npc->getComponent<AIMoveBestRouteComponent>("AIMoveBestRouteComponent");
            if (aiComponent) {
                npcThreads.emplace_back([aiComponent, npc]() {
                    aiComponent->updatePath(Map::GetInstance().buildGraph());
                    });
            }
        }

        for (auto& thread : npcThreads) {
            thread.join();
        }
        npcThreads.clear();
